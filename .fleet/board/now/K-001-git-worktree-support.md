...
<!-- signal: session-2963a257 2026-09-08T06:15Z — Phase 1 implemented and PR'd: chrisjohnson/dsh-deploy#3 (engine 43/43 smoke green; sibling-container boot test green against current image; caught+fixed: tool param 'required' must be true when present, marker dirties worktree (common info/exclude fix), prune skips locked entries). Engine findings to fold into §2 later. -->
<!-- signal: session-2963a257 2026-09-08T07:05Z — design correction per user: the isolation guard is now OPT-IN (default off). A guard denial is monotonic and fires before the sandbox's escalation->approval flow, so default-on removed the user's authority over out-of-workspace writes; the sandbox escalation path already implements "agent asks, user decides". Guard remains for unattended profiles (approval: never) where fail-closed is wanted. -->

<!-- signal: session-2963a257 2026-09-08T17:20Z — adoption PR opened: chrisjohnson/dsh-deploy#4 (feat/wloops-git-worktree, commit 92dd1e1). Sibling boot test GREEN: bundle row composes in dump-config, dsh web serves HTTP 200 on :3081 with 0.4.0 installed. PR #3 closed, feat/dsh-git-worktree-plugin deleted. Post-deploy manual acceptance still open (worktree-rooted session cwd, kill-mid-work recovery, config cleanliness). -->
<!-- signal: session-2963a257 2026-09-08T16:05Z — MAJOR PIVOT per user: custom plugin DISCARDED (PR #3 to be closed, branch deleted). Adopting wloops/dsh-git-worktree@0.4.0 — the only published version built for the dsh 0.1.0-rc.8 line (hard-deps dsh-* 0.1.0-rc.8; 0.7.1/0.7.2 need 0.1.1-rc.2, 0.7.3+ need 0.1.2-rc.1 which adds api-session/workspace-controller packages we lack and drops the optional SQLite session backend). Integration: app+profile package.json dep, bundles list, entrypoint symlink, cordis 4.0.2 override. Boot test in progress. -->
## Decision log
...
- 2026-09-08: Isolation guard flipped to opt-in (enforceIsolation default false) per user correction: the sandbox escalation->approval flow is the user-authority mechanism for out-of-workspace writes; a monotonic guard upstream of it overrules the user. Opt-in for fail-closed unattended profiles.
- 2026-09-08: PR #3 opened (feat/dsh-git-worktree-plugin). New experimental findings to merge into §2: (a) the marker file makes every dsh worktree dirty to git — fixed via common .git/info/exclude (per-worktree info/exclude is honored by nothing on git 2.39.5); (b) `git worktree prune` skips LOCKED entries — the plugin unlocks its own stale entries (lock-reason prefix) before pruning; (c) dsh-tools schema rule: a tool parameter's `required` must be `true` when present (`required: false` is rejected at boot).
...
- 2026-09-08 (PIVOT): User discarded the first-party plugin (PR #3 closed, branch deleted) and directed adopting wloops/dsh-git-worktree. Version archaeology across all 7 npm releases: 0.4.0 (2026-08-20) is the only one built for our dsh 0.1.0-rc.8 line — it hard-deps dsh-* 0.1.0-rc.8 and its client injects (dsh-client-runtime/ui-conversation/ui-tool/api-gateway) all exist in our graph. 0.6.0 hard-deps 0.1.1-rc.2 (would nest a second dsh world); 0.7.x needs the 0.1.2-rc.1 line. Decision: pin 0.4.0 now; revisit 0.7.x if/when the harness line is bumped (the 0.7.x README documents that line's data migration + PTC-mode rename). The first-party engine's findings (marker dirties worktree → common info/exclude; prune skips locked; duplicate-branch rc=255) remain valid prior art for that revisit and for any future hardening of the adopted plugin's behavior.
- 2026-09-09 (i18n contribution): Built the full client-UI i18n PR for wloops/dsh-git-worktree against v0.7.5 (main @ 4a90c37): all ~290 browser-visible strings extracted to a plugin-owned "worktree" locale namespace (zh byte-identical + en translation) via the dsh locale registry (dsh-client-locale bind/register + LocaleNamespaceMap ambient merge); agent prompts and host-protocol literals intentionally kept verbatim. Gates: pnpm typecheck clean, 325/325 tests, build clean - verified twice (working tree + fresh patch apply on a clean worktree). BLOCKED on opening the PR: the GitHub App bot cannot fork/push to wloops/dsh-git-worktree (App tokens have no fork right; App not installed there). Deliverables staged for a human-account push: /work/wloops-dsh-git-worktree-client-i18n.patch (git-apply verified), /work/wloops-i18n-pr-body.md, /work/wloops-i18n-apply-instructions.md (turnkey fork+PR commands). Source branch feat/client-ui-i18n @ 639c639 in /work/wloops-dsh-git-worktree.
- 2026-09-09 (i18n closed out): Upstream (wloops) shipped its own i18n implementation overnight - the prepared PR is no longer needed and was abandoned (nothing was pushed to wloops). Full contribution (patch, PR body, design notes, verification evidence) archived on branch `archive/k-001-wloops-i18n` under .fleet/archive/k-001-wloops-i18n/. Scratch clone /work/wloops-dsh-git-worktree (branch feat/client-ui-i18n @ 639c639) left in place per "otherwise abandon it".
## Handoff notes
- Adoption (wloops/dsh-git-worktree@0.4.0) is wired; verify against the
  acceptance criteria that still apply: (a) a worktree-rooted session
  stays in workspace-write with zero escalations — 0.4.0's console flow
  creates the worktree+session server-side, so confirm the new session's
  SessionHeader.cwd lands on the worktree path; (b) container-kill
  mid-work leaves no blocking state (check its recovery claims against
  this box's /work re-clone behavior); (c) no .git/config poisoning after
  its delivery flow (it creates commits — verify the repo's config stays
  clean). The guard/escalation design questions from the discarded build
  remain open for a future revisit.
- Phase 0 (original): HeathHe/dsh-worktree-panel was never evaluated
  (wloops was adopted instead) — still a candidate if 0.4.0's UI proves
  insufficient.
